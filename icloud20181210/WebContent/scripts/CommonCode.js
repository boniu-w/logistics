...
function ShowTips() {
    ...
    jQuery(".tips").each(function() {
        jQuery(this).before('<span class="tipshelp">&nbsp;&nbsp;</span>');
    });

    jQuery(".tipshelp").hover(function() {
        var tips = jQuery(this).next(".tips").html();
        var tipsContainer = jQuery("#PETipsContainer");
        if (tipsContainer.length > 0) {
            tipsContainer.remove();
        }
        jQuery('<div id="PETipsContainer"><div id="PETips"><div class="triangle" /></div><iframe id="tipsIframe_IE6_Z-Index" width="100%" frameborder="0" height="0" style="position:absolute; top:0px; left:0px; z-index:-1; border-style:none;"></iframe></div>')
        .insertAfter(this)
        .find("#PETips").prepend(tips);
        jQuery("#tipsIframe_IE6_Z-Index").height(jQuery("#PETipsContainer").outerHeight());
        jQuery(this).mousemove(function(e) {
            ...
            if (x + 410 > document.body.clientWidth)
                x = document.body.clientWidth - 410;
            jQuery("#PETipsContainer").css({ "left": x, "top": e.pageY + 18, "display": "block" });
        });
    }, function() {
        jQuery("#PETipsContainer").remove();
    });
}