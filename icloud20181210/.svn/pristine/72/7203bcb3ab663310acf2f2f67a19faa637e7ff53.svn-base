    ...
    render : function (container){
        ...
        for( var i=0; i<this.list.length; i++) {
            ...
            if( this.settings.checkbox || this.settings.radio){
                ...
                inputTemp.innerHTML = '<input '+ (item.checked?'checked':'') +' type="'+(this.settings.checkbox?'checkbox':'radio')+'" name="' + this.settings.name + '" />';
    ...
    renderChild : function(item, checked) {
        ...
        for( var i=0;i<item.children.length;i++) {
            ...
            if( childElem == null) {
                ...
                if( this.settings.checkbox || this.settings.radio){
                    var inputTemp = document.createElement('div');
                    inputTemp.innerHTML = '<input '+ (childItem.checked?'checked':'') +' type="'+(this.settings.checkbox?'checkbox':'radio')+'" name="' + this.settings.name + '" />';
                    var inputElem = inputTemp.childNodes[0];
